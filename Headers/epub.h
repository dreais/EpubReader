...
/**
 * @brief function to read a file from the zip file provided and get the contents
 * @param z zip file to be read
 * @param fname name of the file to be read - file should be present in the zip file
 */
char *read_zfile(struct zip *z, const char *fname);

/**
 * @brief function to get the document for parsing - XML data
 * @param epub_str epub_t structure holding the important details
 * @param content content to be passed for getting the XML document
 */
xmlDocPtr prepare_doc(char *content);

/**
 * @brief function to parse required xml file and load the attributes and filepath pair
 */
